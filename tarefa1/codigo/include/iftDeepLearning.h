...
typedef enum ift_neural_layer_type { IFT_UNKNOWN_LAYER,
	...
	IFT_LOSS_LAYER, IFT_DATASET_LAYER } iftNeuralLayerType;
...
/**
 * @brief Neural net layer parameters. The same structure can represent all different layers. The layer currently instantied is defined in \ref iftNeuralLayer.type parameter.
 * @author Peixinho
 * @date Nov, 2017
 */
...
struct ift_neural_network {
	...
	/**
	 * Number of layers.
	 * Including input and loss layers.
	 */
    int nlayers;

	int curLayer;

	/**Array of neural layers
	 * going from 0 (input layer) to n (output layer)*/
	 ...
	 * and minibatch=ntrain is equivalent to the batch gradient descent.
	 */
    int miniBatch;
...
iftNeuralLayer *iftConvolutionLayer2D(iftNeuralNetwork *net, int xsize, int ysize,
									  ...
									  int xstride, int ystride, int nkernels,
 ...
 */
iftNeuralLayer* iftFullyConnectedLayer(iftNeuralNetwork* net, int ninput, int noutput,
									   iftActivationFunction act, iftActivationFunction deriv);
