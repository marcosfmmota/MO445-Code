...
/**
 * @brief          Read a float image from disk according to the extension type (accepted: .fscn, .npy).
 * @param filename Float Image path.
 * @return         The loaded fimage.
 * 
 * @author Samuka Martins
 * @date Mar 12, 2018
 */
iftFImage *iftFReadImageByExt(const char *filename, ...);
 ...
 *
 */
void iftFWriteImage(const iftFImage *img, const char *filename, ...);


/**
 * @brief          Write a float image on disk according to the extension type (accepted: .fscn, .npy).
 ...
 * @date Mar 12, 2018
 */
void iftFWriteImageByExt(const iftFImage *fimg, const char *filename, ...);
