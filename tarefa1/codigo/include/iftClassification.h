 ...
 *
 * If a testing sample is inside the radius of any training sample, it belongs to the target class and
 * its label will be 1. \n
 * Otherwise, it does not belong to target class, its status is IFT_OUTLIER, and its predicted label will
 * be 2. \n\n
 * 
 * @note Only the samples with status IFT_TEST in Ztest will be classified.
 * 
 * @param graph Trained KnnGraphic (by iftUnsupTrain) with only the training samples of the target class.
 * @param Ztest Dataset for classification (only the samples with status IFT_TEST will be classified) 
 * 
 * @author Samuka Martins
 * @date Feb 21, 2018
 */
void iftClassifyByOneClassOPF(iftKnnGraph *graph, iftDataSet *Ztest);
