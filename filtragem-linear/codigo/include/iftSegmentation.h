...
/**
 * @brief Computes the Watergray Transform from a marker image in a forest strucutre
 * @author
 * @date
 * @ingroup Segmentation
 *
 ...
 * @param img Input Image
 * @param A Adjacency Relation
 * @param mask A mask to constrain the thresholding
...
/**
 * @brief  Different implementation from Filip Malmberg Paper
 *         The BMD distance transforms for multi-channel images should be by computing
 *         the single channel BMD on a randomly chosen channel, and forming a weighted
 *         average of the results, where the weight is the probability of the channel to be chosen.
 *
 * @author Jordão Bragantini
 *
 * @param  mimg             Multi-band image LAB OR GRAYSCALE, other color spaces doesn't produce good results.
 * @param  seeds            Seeds or NULL, if NULL the border will be used as background seeds and each band won't have weight
 * @param  iterations       Number of iterations, more better.
 * @return                  Segmentated Image
 */
iftFImage *iftBooleanMapDistance(iftMImage* mimg, iftLabeledSet* seeds_or_null_for_border, int iterations);
