...
typedef enum ift_sample_status {
    ...
    IFT_ALL             = IFT_UNKNOWN | IFT_TEST | IFT_TRAIN | IFT_OUTLIER | IFT_ERROR |  IFT_ARTIFICIAL | IFT_SUPERVISED | IFT_LABELPROPAGATED | IFT_PROTOTYPE
} iftSampleStatus;
 ...
 */
iftDataSet *iftNormalizeDataSetByZScore(iftDataSet *Z, iftFeatSpaceParam *fsp);
 ...
 * @param true_label: If true is maps truelabel, else label
 * @return The label image.
 */
//! swig(newobject)
iftImage *iftDataSetToLabelImage(const iftDataSet *Z, const iftImage *comp, bool decrement_labels, bool true_label);
 ...
 * @date Jul 8, 2018.
 */
inline bool iftHasSampleStatus(iftSample sample, iftSampleStatus status) {
 ...
 * @date Jun 8, 2018
 */
inline void iftSetSampleStatus(iftSample *sample, iftSampleStatus status) {
 ...
 *
 * @note The current sample statuses are kept.
 * @warning Be careful when the status is IFT_TRAIN, because this function does not update
 * the parameter ntrainsamples from iftDataSet.
 *
 * @author Samuka Martins
 * @date Jun 8, 2018
 */
inline void iftAddSampleStatus(iftSample *sample, iftSampleStatus status) {
    sample->status |= status;
}
 ...
 *
 * @note The remaining sample statuses are kept.
 * @warning Be careful when the status is IFT_TRAIN, because this function does not update
 * the parameter ntrainsamples from iftDataSet.
 *
 * @author Samuka Martins
 * @date Jun 8, 2018
 */
inline void iftRemoveSampleStatus(iftSample *sample, iftSampleStatus status) {
    sample->status &= (~status);
}
 ...
 */
//! swig(newobject)
iftDataSet *iftMImageToDataSet(const iftMImage *mimg, const iftImage *label_img);
