...
//! swig(extend = iftHistogramExt.i, destroyer = iftDestroyHistogram)
...
/**
 * @brief Computes the histogram of pixel intensities for the pixels inside a region of interest.
 *
 * @author Thiago Vallin Spina
 * @date Feb 21, 2016
 *
 * @param img The input grayscale image.
 * @param bin Region of Interest. If NULL, the entire image is considered.
 ...
 *""""
 */
iftHistogram *iftGrayHistogramInRegion(const iftImage *img, const iftImage *bin, int nbins, bool normalize);
 ...
 * @date Aug 24, 2018
 *
 * @note The number of histograms is 1 + max_val, where max_val is the maximum value of the label image.
 * Thus, the histogram[x] correspondts to that for the object with label x.
 * 
 * @param img       The input grayscale image.
 * @param label_img The Label Image.
 * @param nbins     The number of bins into which the histogram should be divided.
 *                  Note: the minimum and maximum intensity of the image are considered for defining the bins.
 * @param normalize If true, normalizes the histogram by dividing it by the number of pixels in the binary mask.
 ...
 */
iftHistogram **iftGrayHistogramsForLabels(const iftImage *img, const iftImage *label_img, int nbins,
                                          bool normalize, int *n_hists_out);
...
iftHistogram *iftNormalizeHistogram(iftHistogram *src);
