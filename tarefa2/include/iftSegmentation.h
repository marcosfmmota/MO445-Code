 ...
 */
typedef enum {
    IFT_MASK_JOIN_OPERATION_NONE, IFT_MASK_JOIN_OPERATION_UNION, IFT_MASK_JOIN_OPERATION_INTERSECTION
 ...
 * @param highVal: Value to represent the mask pixels (e.g. 1 or 255)
 *
 * @return Joint mask
 */
iftImage *iftCreateJointMaskFromFileDir(char *maskDir, iftMaskJoinOperation operation, int highVal);

//! swig(newobject)
iftImage *iftMahalanobisWaterCut(iftMImage *mimg, iftMatrix *M, iftAdjRel *Ain, iftLabeledSet *seeds);
