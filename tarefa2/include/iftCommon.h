 ...
 * @date Jun 28, 2018
 */
#define iftInInterval(x, a, b) (( (x) >= (a) && (x) <= (b) ))
...
#define iftSquaredVoxelDistance(u,v) ((float) ((u.x-v.x)*(u.x-v.x)+(u.y-v.y)*(u.y-v.y)+(u.z-v.z)*(u.z-v.z)))
...
void iftWriteIntArray(iftIntArray *array, char *filename);
iftIntArray *iftReadIntArray(const char *filename);
 ...
 * @author Cesar Castelo
 * @date Feb 01, 2018
 * @brief Searches a string in a string array
 * @param array Array to search the elem
 * @param elem String to search in the array
 * @return Position of the element inside the array or -1 if it was not found
 * @return
 */
int iftFindStrArrayElementIndex(iftStrArray *array, char *elem);
 ...
 * @return The element's index in <x> or NIL if not found
 */
int iftMaxValIntArray(int *array, int n);
